...
// };
use constants::{
    GOVERNANCE_BOT_USER_NAME, GOVERNANCE_CANISTER_ID, INITIAL_PROPOSAL_LIMIT, PROCESS_INTERVAL,
    TOPIC_0_GROUP_CANISTER_ID, TOPIC_10_GROUP_CANISTER_ID, TOPIC_1_GROUP_CANISTER_ID,
    TOPIC_2_GROUP_CANISTER_ID, TOPIC_3_GROUP_CANISTER_ID, TOPIC_4_GROUP_CANISTER_ID,
    TOPIC_5_GROUP_CANISTER_ID, TOPIC_6_GROUP_CANISTER_ID, TOPIC_7_GROUP_CANISTER_ID,
    TOPIC_8_GROUP_CANISTER_ID, TOPIC_9_GROUP_CANISTER_ID,
};
use ic_cdk::api::time;
use ic_cdk_macros::{heartbeat, query};
use std::cell::RefCell;
use types::{
    ListProposalInfo, ListProposalInfoResponse, MessageContent, Proposal, ProposalInfo,
    SendMessageArgs, SendMessageResponse, TextContent,
};
...
fn previous_process_time() -> u64 {
    PREVIOUS_PROCESS_TIME_REF_CELL
        .with(|previous_process_time_ref_cell| *previous_process_time_ref_cell.borrow())
}

#[query]
fn last_proposal_id_sent() -> u64 {
    LAST_PROPOSAL_ID_SENT_REF_CELL
        .with(|last_proposal_id_sent_ref_cell| *last_proposal_id_sent_ref_cell.borrow())
}

#[query]
fn list_proposals_error() -> (u64, String) {
    LIST_PROPOSALS_ERROR_REF_CELL
        .with(|list_proposals_error_ref_cell| list_proposals_error_ref_cell.borrow().clone())
}

#[query]
fn send_message_response() -> (u64, String) {
    SEND_MESSAGE_RESPONSE_REF_CELL
        .with(|send_message_response_ref_cell| send_message_response_ref_cell.borrow().clone())
}
...
fn process_proposals() {
    ic_cdk::spawn(async {
        ...
        match proposals_infos_result {
            ...
            Err(error) => LIST_PROPOSALS_ERROR_REF_CELL.with(|list_proposals_error_ref_cell| {
...
async fn get_proposal_infos(limit: u32) -> Result<Vec<ProposalInfo>, String> {
    // 取出上个提案 id
    let last_proposal_id_sent = LAST_PROPOSAL_ID_SENT_REF_CELL
        .with(|last_proposal_id_sent_ref_cell| *last_proposal_id_sent_ref_cell.borrow());

    let mut proposal_infos = list_proposals(limit).await?; // 取出上一个提案信息
    let mut index = 1;

    while proposal_infos_contains_last_proposal(last_proposal_id_sent, &proposal_infos) == false {
        // 不包含的情况下，需要再次调用取出最近的提案信息
        proposal_infos = list_proposals(limit + index).await?;
        index += 1;
    }

    let mut proposal_infos_without_last_proposal =
        remove_proposal_from_proposal_infos(last_proposal_id_sent, proposal_infos);

    proposal_infos_without_last_proposal.reverse(); // 倒序一下
...
async fn process_proposal_infos(proposal_infos: Vec<ProposalInfo>) {
    for proposal_info in proposal_infos.iter() {
        if let Some(neuron_id) = &proposal_info.id {
            process_proposal_info(neuron_id.id as u128, proposal_info.clone()).await;
            ...
            LAST_PROPOSAL_ID_SENT_REF_CELL.with(|last_proposal_id_sent_ref_cell| {
                ...
                *last_proposal_id_sent_ref = neuron_id.id; // 将处理过的提案 id 赋值成为上次的 id
            });
        }
    }
}
...
// 处理一个提案信息
async fn process_proposal_info(neuron_id: u128, proposal_info: ProposalInfo) {
    let canister_id = get_group_canister_id(proposal_info.topic); // 取得对应的主题群 id

    let call_result = send_message_to_group(
        &canister_id,
        neuron_id,
        &format_proposal_message(neuron_id, proposal_info.proposal),
    )
    .await;
...
// 格式化提案信息
fn format_proposal_message(id: u128, proposal_option: Option<Proposal>) -> String {
    if let Some(proposal) = proposal_option {
        ...
        if proposal_message.len() > 1000 {
            proposal_message.truncate(1000); // 截取前 1000 位字符
